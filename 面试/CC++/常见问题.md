...
+ static_cast 类似于C语言的类型转换
+ dynamic_cast 用于父类指针和子类指针的转换，子类转父类安全，父类转子类不安全
+ const_cast 用于const关键字的去除
+ reinterpret_cast 类似于C语言的显示强制转换，不检测安全，慎用

## 54、遇到比较难解决的问题

在使用libevent实现websocket时，有时会出现一个线程在发送数据，另一个线程把连接断开，出现崩溃的情况。在网上查了一下，说libevent在多线程使用需要在初始化前加evthread_use_pthreads()，但加了后，还是有问题。后来在看muduo的c++网络库，他里面说到，对IO的操作应该放在IO线程中进行，这样才能保证线程安全，因此把连接关闭的操作放入IO线程后，问题解决了
